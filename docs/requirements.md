...
#### 出品禁止物の自動検出（優先度：高）
- 出品時にタイトル・説明文をAIで審査
- 禁止ワード（刃物、医薬品等）をプロンプトで検出
- 警告表示 or 出品ブロック
- エンドポイント: `POST /api/ai/check-item`

#### AI 価格提案（優先度：中）
- タイトル・カテゴリ・状態から適正価格をAIが提案
- 出品フォームに「価格を提案してもらう」ボタン
- エンドポイント: `POST /api/ai/suggest-price`

#### テスト整備（優先度：低）
- バックエンドのユニットテスト（`*_test.go`）
- 主要APIのハンドラーテスト
